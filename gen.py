...
def gen_res(out_dir,
    ...
    replace(out_dir + "/cmpf.py", "cv_dih_dim = .*",
            "cv_dih_dim = %d" % cv_dim_list[0])
...
def gen_rid(out_dir,
    ...
    # template/00.enhcMD
    replace(out_dir + "/template/00.enhcMD/test.std.py",
            "cv_dim = .*", "cv_dim = %d" % cv_dim)
    ...
    # template/01.resMD
    gen_res(out_dir + "/template/01.resMD",
            gen_file, cv_file, mol_dir, res_dir)

    # template/02.train
    replace(out_dir + "/template/02.train/model.py",
            "cv_dim = .*", "cv_dim = %d" % cv_dim)
    replace(out_dir + "/template/02.train/model.py",
            "cv_dih_dim = .*", "cv_dih_dim = %d" % cv_dim_list[0])
    replace(out_dir + "/template/02.train/model.py",
            "cv_dist_dim = .*", "cv_dist_dim = %d" % cv_dim_list[1])

    # template/mol
    gen_bf(out_dir + "/template/mol", gen_file, cv_file, mol_dir)

    # template/tools
    replace(out_dir + "/template/tools/cluster_cv.py",
            "cv_dih_dim = .*", "cv_dih_dim = %d" % cv_dim_list[0])


def gen_rid_sits(out_dir,
                 gen_file,
                 cv_file,
                 mol_dir,
                 res_dir,
...
def gen_strm(out_dir,
    ...
    # template/00.resMD
    gen_res(out_dir + "/template/00.resMD",
            json_file, mol_dir, res_dir, tools_dir)

    # template/mol
    gen_bf(out_dir + "/template/mol", json_file, mol_dir, tools_dir)
    shutil.copy(tools_dir + "conf.ang.sh", out_dir + "/template/mol")
    replace(out_dir + "/template/mol/conf.ang.sh",
            "nalpha=.*", "nalpha=%d" % nalpha)
    replace(out_dir + "/template/mol/conf.ang.sh",
            "nangle=.*", "nangle=%d" % nangle)
...
def _main():
    ...
    if args.TASK == "bf":
        ...
        gen_rit(args.output, args.GEN_DEF,
                args.CV_DEF, mol_dir, res_dir, rit_dir)
    # elif args.TASK == "strm" :
