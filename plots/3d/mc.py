...
def _main():
    ...
    fp = open("angle.dat", "a")
    xx = pylab.linspace(-np.pi, np.pi, 100)
    yy = pylab.linspace(-np.pi, np.pi, 100)
    pp_hist = np.zeros((fd // 2, len(xx), len(yy)))
    delta = 2.0 * np.pi / 100.0
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for ii in range(ns):
            pp, ee, ff = walker.sample(compute_ef)
            pp_hist_new = my_hist(pp, xx, yy, delta, fd // 2)
            pp_hist = (pp_hist * ii + pp_hist_new) / (ii + 1)

            if np.mod(ii, 10000) == 0:
                zz = np.transpose(-np.log(pp_hist + 1e-6), (0, 2, 1)) / beta
                # zz = zz - np.min(zz)
                zz *= f_cvt
                plt.figure(figsize=(25, 4.5))
                for jj in range(fd // 2):
                    plt.subplot(1, fd // 2, jj + 1)
                    plt.pcolor(xx, yy, zz[jj, :, :])
                    plt.colorbar()
                    plt.hold(True)
                    plt.contour(xx, yy, zz[jj, :, :])
                    plt.text(-3, 2, "angle_pair.%02d" % (jj))
                plt.savefig("data_pdf.png")
                plt.show()
            # np.savetxt(fp,np.array(pp[0,:]))
            # fp.write()
            # fp.flush()
