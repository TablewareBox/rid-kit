...
def parse_arg():
    parser = argparse.ArgumentParser(
        description='This is a program to plot each CV as time, python plot_eachCV_one.py -infiledir ./ -num_iter 100 '
                    '-num_walkers 12 -cv_index 8 -time_len 2 -outfile CV8.png')
...
def plot_iter(filedir, num_iter, num_walkers, cv_index, time_len, figout):
    ...
    for wal in range(int(num_walkers)):
        ...
        for it in range(int(num_iter)):
            ...
            filename2 = filedir + "iter." + str(iteration) + "/00.enhcMD/" + str(walker_name) + "/trust_lvl1.dat"
            data = np.loadtxt(filename)
            lvl = np.loadtxt(filename2)
            lvl_list = np.zeros(len(data)) + lvl
            time = data[:, 0] / 1000 + it * float(time_len)
            ang = data[:, int(cv_index)]
            all_cv.extend(list(ang))
            all_time.extend(list(time))
            all_lvl.extend(list(lvl_list))
            # if np.max(ang)>4:
            #    print(filename)
        all_cv = np.array(all_cv)
        all_cv[all_cv < 0] += 2 * np.pi
        sub = fig.add_subplot(int(num_walkers) / 4, 4, wal + 1)
        sub.scatter(all_time, all_cv, s=1)
        sub.plot(all_time, all_lvl, lw=1.2, alpha=0.8, color='red')
        sub.set_ylabel('angle (rad)', fontproperties=font_prop)
        sub.set_xlabel('time (ns)', fontproperties=font_prop)
        plt.ylim(0, 2 * np.pi)
