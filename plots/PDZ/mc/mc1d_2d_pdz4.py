...
class Walker(object):
    def __init__(self, fd_dih, fd_dist, nw, sess):
        ...
        self._shape11 = (self._num_walker, 1)

        self._shape1_ = (self._num_walker - 10, self._full_dim_dih)
        self._shape2_ = (self._num_walker - 10, self._full_dim_dist)
        self._shape11_ = (self._num_walker - 10, 1)
        self._shape_cry = (10, self._full_dim)
        # absolute coordinate
        cry = np.loadtxt('crystal.cv')

        self._position_dist1 = np.random.uniform(0.3, 3.8, size=self._shape11_)
        self._position_dist0 = self._position_dist1 + np.random.normal(loc=-0.1, scale=0.4, size=self._shape11_)
        self._position_dist2 = self._position_dist1 + np.random.normal(loc=-0.1, scale=0.4, size=self._shape11_)
        self._position_dist0[self._position_dist0 < 0.45] = 0.49 + np.random.normal(scale=0.01)
        self._position_dist0[self._position_dist0 > 3.79] = 3.73 + np.random.normal(scale=0.01)
        self._position_dist2[self._position_dist2 < 0.45] = 0.49 + np.random.normal(scale=0.01)
        self._position_dist2[self._position_dist2 > 3.79] = 3.73 + np.random.normal(scale=0.01)

        self._position1 = np.concatenate((np.random.uniform(size=self._shape1_) * np.pi * 2, self._position_dist0,
                                          self._position_dist1, self._position_dist2), axis=1)
        self._position2 = np.random.normal(size=self._shape_cry, loc=cry, scale=0.03)
        self._position = np.concatenate((self._position1, self._position2), axis=0)

        self._energy = np.zeros([self._num_walker])
        self._force = np.zeros([self._num_walker, self._full_dim])
        self._sample_step = 20
        self._acp_ratio_lb = 0.14
    ...
    def sample(self, compute_ef, inter_step=1):
        ...
        for _ in range(inter_step):
            dist1 = self._position[:, self._full_dim_dih + 1].reshape(-1, 1) + np.random.normal(
                scale=self._move_scale * 0.4, size=self._shape11)
            dist1[dist1 < 0.3] = 0.37 + np.random.normal(scale=0.02)
            dist1[dist1 > 3.79] = 3.72 + np.random.normal(scale=0.02)

            dist0 = self._position[:, self._full_dim_dih].reshape(-1, 1) + np.random.normal(
                scale=self._move_scale * 0.4, size=self._shape11)
            dist0[dist0 - dist1 < -1.5] = dist1[dist0 - dist1 < -1.5] - 1.5
            dist0[dist0 - dist1 > 1.3] = dist1[dist0 - dist1 > 1.3] + 1.3
            dist0[dist0 < 0.45] = 0.49 + np.random.normal(scale=0.01)
            dist0[dist0 > 3.79] = 3.73 + np.random.normal(scale=0.01)

            dist2 = self._position[:, self._full_dim_dih + 2].reshape(-1, 1) + np.random.normal(
                scale=self._move_scale * 0.4, size=self._shape11)
            dist2[dist1 - dist2 < -1.5] = dist1[dist1 - dist2 < -1.5] + 1.5
            dist2[dist1 - dist2 > 1.0] = dist1[dist1 - dist2 > 1.0] - 1.0
            dist2[dist2 < 0.45] = 0.49 + np.random.normal(scale=0.01)
            dist2[dist2 > 3.79] = 3.73 + np.random.normal(scale=0.01)

            position_new = np.concatenate((np.mod(
                self._position[:, 0:self._full_dim_dih] + np.random.normal(scale=self._move_scale, size=self._shape1),
                2 * np.pi), dist0, dist1, dist2), axis=1)
...
def my_hist2d(pp, xx, yy, delta1, delta2, fd1, fd2):
    ...
    for ii in range(pp.shape[0]):
        ...
        my_hist[0, np.int(singleSD(pp[ii, 0:fd1] / 3.14 * 180, dataCry[0:fd1]) // delta1), np.int(
            np.mean(pp[ii, fd1:fd1 + fd2]) // delta2)] += 1
...
def _main():
    ...
    parser.add_argument("-nw", "--num_walker", default=3000, type=int,
