...
class Walker(object):
    def __init__(self, fd_dih, fd_dist, nw, sess):
        ...
        self._shape1_ = (self._num_walker - 100, self._full_dim_dih)
        self._shape2_ = (self._num_walker - 100, self._full_dim_dist)
        self._shape11_ = (self._num_walker - 100, 1)
        ...
        self._position_dist0 = self._position_dist1 + np.random.normal(loc=-0.1, scale=0.38, size=self._shape11_)
        self._position_dist2 = self._position_dist1 + np.random.normal(loc=-0.1, scale=0.38, size=self._shape11_)
        self._position_dist0[self._position_dist0 < 0.45] = 0.52 + np.random.normal(scale=0.02)
        self._position_dist0[self._position_dist0 > 3.79] = 3.6 + np.random.normal(scale=0.06)
        self._position_dist2[self._position_dist2 < 0.45] = 0.52 + np.random.normal(scale=0.02)
        self._position_dist2[self._position_dist2 > 3.79] = 3.6 + np.random.normal(scale=0.06)
        ...
        self._position2 = np.random.normal(size=self._shape_cry, loc=cry, scale=0.02)
    ...
    def sample(self, compute_ef, inter_step=1):
        ...
        for _ in range(inter_step):
            dist1 = self._position[:, self._full_dim_dih + 1].reshape(-1, 1) + np.random.normal(
                scale=self._move_scale * 0.08, size=self._shape11)
            dist1[dist1 < 0.3] = 0.33 + np.random.normal(scale=0.01)
            dist1[dist1 > 3.79] = 3.76 + np.random.normal(scale=0.01)

            dist0 = dist1 + np.random.normal(loc=-0.1, scale=0.36, size=self._shape11)
            dist0[dist0 < 0.45] = 0.48 + np.random.normal(scale=0.01)
            dist0[dist0 > 3.79] = 3.72 + np.random.normal(scale=0.02)

            dist2 = dist1 + np.random.normal(loc=-0.1, scale=0.36, size=self._shape11)
            dist2[dist2 < 0.45] = 0.48 + np.random.normal(scale=0.01)
            dist2[dist2 > 3.79] = 3.72 + np.random.normal(scale=0.02)
